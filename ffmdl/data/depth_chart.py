...
def init():
    """
    Load environment variables and redis db
    """
...
def create_url(team_name):
    """
    Create url to get depth chart table from
    """
    team_name = common.exception_dict.get(team_name, team_name)
    return f"https://www.{team_name}.com/team/depth-chart"
    ...
    for char in player_list:
        ...
        if char == '\n':
            ...
            if pos_check in positions:
                ...
                # if it's NOT the first position string encountered,
                # and we have a non-empty list of players
                # then copy the list (otherwise its a reference to the same list),
                # and then place it in dictionary
...
def mt_update(team_db):
    """
    main function to update the db of teams
    """
