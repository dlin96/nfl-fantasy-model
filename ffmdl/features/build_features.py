...
def build_base_df():
    ...
    db_host = os.getenv("nfldb_host")

    # connect to nfldb
    db_uri = "postgresql://{username}:{password}@{host}:{port}/nfldb"
    engine = sqlalchemy.create_engine(
        db_uri.format(username=db_user, password=db_pass, host=db_host, port=db_port))
    conn = engine.connect()
...
def zp_scores(position, df):
    ...
    pos_df["p_score"] = st.norm.cdf(pos_df["z_score"])
    return pos_df.rename(columns={"y": "pred_avg_pts"})


if __name__ == '__main__':
    build_base_df().to_pickle("../../data/interim/base_fantasy_df.pkl")
